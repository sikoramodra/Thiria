...
<div class="flex items-center justify-center flex-col bg-firstColor" style="height: calc( 100vh - 6rem )">
    <h1 class="font-bold font-times text-8xl">{{$user->name}}</h1>
    <h2 class="font-bold font-times text-2xl">Member since {{$user->created_at->format('Y-m-d')}}</h2>

        <!--<div type="submit" name="vote" class="py-1.5 px-3 hover:text-green-600 hover:scale-105 hover:shadow text-center border border-gray-300 rounded-md h-8 text-sm flex items-center gap-1 lg:gap-2">
        ...
        <div type="submit" name="vote" class="py-1.5 px-3 hover:text-red-600 hover:scale-105 hover:shadow text-center border border-gray-300 rounded-md h-8 text-sm flex items-center gap-1 lg:gap-2">
            <svg class="w-8 h-8" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
    ...
    <div>
        <h2 class="font-bold font-times text-3xl ml-12 mt-24">{{$user->name}}'s creatures:</h2>
        <div class="grid grid-cols-2 md:grid-cols-3 gap-4 p-4 overflow-y-scroll shadow-2xl shadow-black" style="height: calc( 100vh - 24rem); scrollbar-width:initial; scrollbar-color: #6C584C transparent;">
            @foreach ($creatures as $creature)
                <a href="{{ route('creature.view_show', ['creature' => $creature]) }}">
