...
<div class="px-32 py-8 flex flex-col overflow-y-scroll bg-firstColor bg-opacity-50 backdrop-blur-md font-times" style="height: calc( 100vh - 6rem ); scrollbar-width:initial; scrollbar-color: #6C584C transparent;">
    
    <h3 class="font-bold text-8xl ">{{ $creature->name }}</h3>
    <h3 class="font-bold text-2xl ml-8">
    ...
    <h2 class="font-bold text-3xl mt-8">{{ $creature->description }}</h2>
        ...
        @if (Auth::id()==$creature->user->id)
            <div class="my-12 flex gap-4">
                <a href="{{ route('creature.edit', ['creature' => $creature]) }}">
                    <button type="submit" class="w-20 text-fifthColor bg-thirdColor hover:bg-secondColor font-medium rounded-lg text-sm px-5 py-2.5 text-center">Edit</button>
                </a>
    ...
    <div>
        <form action="{{ route('comment.add') }}" method="post" class="flex flex-col gap-2 my-8">
            @csrf
            <h3 id="comment_label" class="text-black text-2xl">Comment</h3>
            @error('text')
            ...
            <div class="my-4 flex gap-4">
                <button type="submit" id="submit_button" class="w-20 text-fifthColor bg-thirdColor hover:bg-secondColor font-medium rounded-lg text-sm py-2.5 text-center">Comment</button>
                <button type="button" id="cancel_respond" style="display: none;" class="w-20 text-fifthColor bg-thirdColor hover:bg-secondColor font-medium rounded-lg text-sm py-2.5 text-center" onclick="cancelRespond()">Cancel<i data-feather="x"></i></button>
            </div>
        </form>
    ...
    <div>
        <div class="overflow-y-scroll" style="height: calc( 100vh - 24rem); scrollbar-width:initial; scrollbar-color: #6C584C transparent;">
